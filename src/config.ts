...
const resolvePagesDirPath = async ({
  ...
  const pagesPathCandidates = [path.resolve("pages"), path.resolve("src/pages")];
  ...
  throw new Error('"pages" directory is not found. Please try to specify "pagesDirPath" config.');
};

export const loadConfig = async (configFilePath: string | undefined): Promise<Required<Config>> => {
